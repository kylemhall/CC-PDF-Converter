...
void CCLicenseWizTypePage::SetDescription(int nSelected)
{
	if (nSelected == -1)
		...
		SetDlgItemText(IDC_DESCRIPTION, (RemoveHtmlTags(LoadResourceString(IDS_DESCRIPTION_CC + (int)LicenseInfo::LTUnknown))).c_str());
	else
		// Set description for the license
		SetDlgItemText(IDC_DESCRIPTION, (RemoveHtmlTags(LoadResourceString(IDS_DESCRIPTION_CC + nSelected))).c_str());
}
