...
{
	//DWORD dw = MAKEWPARAM(XtoPercent(m_rectLocation.left), YtoPercent(m_rectLocation.top));
	DWORD dw = MAKEWPARAM(m_rectLocation.left, m_rectLocation.top);
	::PostMessage(GetParent(m_hWnd), UM_LICENSEMOVED, 0, dw);
}

/**
	@param uX The new X location of the stamp (in page co-ordinates)
*/
void CCLicenseLocationWnd::SetXLocation(UINT uX)
{
	uX = XfromPercent(uX);
	// Make sure the stamp stays in the page
	if ((long)uX > m_szPage.cx - (m_rectLocation.right - m_rectLocation.left))
...
/**
	@param uY The new Y location of the stamp (in page co-ordinates)
*/
void CCLicenseLocationWnd::SetYLocation(UINT uY)
{
	uY = YfromPercent(uY);
	// Make sure the stamp stays in the page
	if ((long)uY > m_szPage.cy - (m_rectLocation.bottom - m_rectLocation.top))
...
//////////////////////////////////////////////////////////////////////////
LONG CCLicenseLocationWnd::XtoPercent (LONG val) const
{
	LONG range = m_szPage.cx - (m_rectLocation.right-m_rectLocation.left);
	return round(100.0*val/range);
}
//////////////////////////////////////////////////////////////////////////

LONG CCLicenseLocationWnd::XfromPercent (LONG val) const
{
	LONG range = m_szPage.cx - (m_rectLocation.right-m_rectLocation.left);
	return round(1.0*range*val/100.0);
}
//////////////////////////////////////////////////////////////////////////

LONG CCLicenseLocationWnd::YtoPercent (LONG val) const
{
	LONG range = m_szPage.cy - (m_rectLocation.bottom-m_rectLocation.top); 
	return round(100.0*val/range);
}
//////////////////////////////////////////////////////////////////////////

LONG CCLicenseLocationWnd::YfromPercent (LONG val) const
{
	LONG range = m_szPage.cy - (m_rectLocation.bottom-m_rectLocation.top);
	return round(1.0*range*val/100.0);
}
