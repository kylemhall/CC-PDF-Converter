...
/*
 * Copyright (C) 2007-2010 Guy Hachlili <hguy@cogniview.com>
 * 
 * Used with permission in CC PDF Converter and Excel to PDF Converter
 *
 */
...
public:
	...
	*/
	RealRecord* GetRecord(int nNum) const {if ((nNum < 0) || (nNum >= (int)m_arRecords.size())) return NULL; return m_arRecords[nNum];};
	/**
		@brief Retrieves the number of records in the set
		@return The number of records
	*/
	int			GetRecordCount() const {return (int)m_arRecords.size();};
	/**
	...
	*/
	int			GetColumnCount() const {return (int)m_mColumns.size();};
	/**
...
{
	ASSERT(m_nRef == 0);
	for (int i=0;i < (int)m_arRecords.size();i++) 
...
LPCTSTR RealTransaction::TRANSACTION_TYPE[] =
{
	_T("DEFERRED"),
	_T("IMMEDIATE"),
	_T("EXCLUSIVE"),
...
public:
	...
	*/
	...
	/**
		...
		@return true if set successfully, false if failed
	*/
	bool		BindParameter(int nParam, LPCTSTR pValue) {if (!IsValid()) return false; if (m_bExecuted) if (!Reset()) return false; std::string sValue = MakeAnsiString(pValue); if (sqlite3_bind_text(m_pStatement, nParam, sValue.c_str(), (int)sValue.size(), SQLITE_TRANSIENT) == SQLITE_OK) return true; m_sLastError = m_db.GetLastError(); return false;};
	/**
		@brief Sets a parameter's value in the statement (by index)
		@param nParam The parameter's index
		@param nValue The parameter's new value
