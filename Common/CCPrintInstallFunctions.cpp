...
static LPCTSTR pPrinterFilelist[] =
{
	...
	_T("CCPDFPrn.PPD"),
	_T("CCPDFConverter.ini"),
	_T("CCPrintUI.DLL"),
	_T("CCPSRendering.DLL"),
#elif EXCEL_TO_PDF
	_T("XLPDFPrn.PPD"),
...
bool CCPDFPrinterInstaller::DoInstall(HMODULE hModule /* = NULL */,  LPTSTR installationPath /*= ""*/)
{
	...
	_tcscat_s(cPath, _S(cPath), m_bX64? _T("DriverFiles64bit") : _T("DriverFiles"));
	// Start installing:
...
bool CCPDFPrinterInstaller::DoRemove()
{
	...
	{
		_stprintf_s(cError, _S(cError), _T("Printer removal error %d (%d)"), GetError(), GetLastError());
		ShowError(cError);
		return false;
	}
