...
public:
	...
	*/
	LicenseInfo(const LicenseInfo& info) : m_eLicense(info.m_eLicense), m_bCommercialUse(info.m_bCommercialUse), m_eModification(info.m_eModification), m_eSampling(info.m_eSampling) {_tcscpy_s(m_cJurisdiction, info.m_cJurisdiction); _tcscpy_s(m_cURI, info.m_cURI); _tcscpy_s(m_cName, info.m_cName);};
