...
#define ENVIRONMENT_64			_T("Windows x64")
...
/**
	...
	@return true if a file (not folder) exists in the path
*/
...
PVOID Disable64Redirection ()
{
	HMODULE hUser = LoadLibrary(_T("Kernel32.dll"));
	if (hUser == NULL)
		return NULL;

	PVOID OldValue = NULL;
	typedef BOOL (__stdcall *fptype)(__out  PVOID);
	fptype fp = (fptype)GetProcAddress(hUser, "Wow64DisableWow64FsRedirection");
	if (fp != NULL)
		fp (OldValue);
//	else
//		MessageBox (NULL, "Failed to find Wow64DisableWow64FsRedirection!", "", 0);
...
BOOL Revert64Redirection (PVOID oldval)
{
	HMODULE hUser = LoadLibrary(_T("Kernel32.dll"));
	if (hUser == NULL)
		return NULL;
	...
	BOOL ret;
	typedef BOOL (__stdcall *fptype)(__in  PVOID);
	fptype fp = (fptype)GetProcAddress(hUser, "Wow64RevertWow64FsRedirection");
	if (fp != NULL)
		ret = fp (oldval);
//	else
//		MessageBox (NULL, "Failed to find Wow64RevertWow64FsRedirection!", "", 0);
...
bool PrinterInstall::DoInstallPrinterDriver()
{
	...
	_tcscpy_s(cDriverPath, _S(cDriverPath), lpPath);
	_tcscat_s(cDriverPath, _S(cDriverPath), m_arFiles[DRIVER_FILE_NUM].c_str());
	// ... the printer UI file ...
	_tcscpy_s(cConfigPath, _S(cConfigPath), lpPath);
	_tcscat_s(cConfigPath, _S(cConfigPath), m_arFiles[UI_FILE_NUM].c_str());
	// ... the printer help file ...
	_tcscpy_s(cHelpPath, _S(cHelpPath), lpPath);
	_tcscat_s(cHelpPath, _S(cHelpPath), m_arFiles[HELP_FILE_NUM].c_str());
	// ... and the printer configuration file
	_tcscpy_s(cDataPath, _S(cDataPath), lpPath);
	_tcscat_s(cDataPath, _S(cDataPath), m_arFiles[DATA_FILE_NUM].c_str());

	// Add all the other files into a buffer
	int nSize = 0;
	STRARRAY::const_iterator i;
	for (i = m_arFiles.begin(); i != m_arFiles.end(); i++)
		nSize += ((*i).size() + 1);
	nSize++;
	...
	info.pEnvironment = m_bX64 ? ENVIRONMENT_64 : ENVIRONMENT;
...
{
	// Try to uninstall
	if (!::DeletePrinterDriverEx(NULL, m_bX64? ENVIRONMENT_64: ENVIRONMENT, (TCHAR*)m_sDriverName.c_str(), DPD_DELETE_UNUSED_FILES, 3)) 
	{
...
{
	// Due to certain stupid Windows 7 mechanisms, we need to disable this thing:
	PVOID oldval = Disable64Redirection();

	// Only add the port monitor if it's not already installed:
	DWORD dwNeeded, dwReturned;
	::EnumMonitors(NULL, 1, NULL, 0, &dwNeeded, &dwReturned);
	if (dwNeeded > 0)
	{
		MONITOR_INFO_1* pMonitors = (MONITOR_INFO_1*)new char[dwNeeded];
		bool bFound = false;
		if (::EnumMonitors(NULL, 1, (LPBYTE)pMonitors, dwNeeded, &dwNeeded, &dwReturned))
		{
			for (DWORD i=0;!bFound && (i<dwReturned);i++)
				bFound = _tcscmp(pMonitors[i].pName, lpMonitorName) == 0;
		}
		delete [] pMonitors;
		if (bFound)
			return true;
...
{
	// Try to delete
	if (!::DeleteMonitor(NULL, m_bX64? ENVIRONMENT_64: ENVIRONMENT, (TCHAR*)lpMonitorName))
	{
		// Did we actually fail?
		DWORD dw = GetLastError();
		if (dw != 0)
