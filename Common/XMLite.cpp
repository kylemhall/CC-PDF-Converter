...
// bro      2002-10-29
//========================================================
int _tcsenicmp( LPCTSTR psz, LPCTSTR str, std::tstring::size_type len, int escape )
{
	LPTSTR pch = (LPTSTR)psz;
	LPTSTR prev_escape = NULL;
...
// bro      2002-10-29
//========================================================
LPTSTR _tcsenistr( LPCTSTR psz, LPCTSTR str, std::tstring::size_type len, int escape )
{
	LPTSTR pch = (LPTSTR)psz;
	LPTSTR prev_escape = NULL;
	LPTSTR des = (LPTSTR)str;
...
{
	std::tstring::size_type len = _tcslen( str );
...
void _SetString( LPTSTR psz, LPTSTR end, std::tstring* ps, bool trim = FALSE, int escape = 0 )
{
	...
	std::tstring::size_type len = (end - psz);
	if( len <= 0 ) return;
	if( escape )
	{
...
{
	std::tstring sRet;
	std::tstring::size_type i, iStart;
	std::tstring::size_type nLen = (int)value.size();
	std::tstring s;
	WCHAR w[2];
	w[1] = '\0';
...
// bro      2002-12-26
//========================================================
std::tstring::size_type	_tagXMLNode::GetChildCount()
{
	return childs.size();
}
...
std::tstring::size_type _tagXMLEntitys::Ref2Entity( LPCTSTR estr, std::tstring& str, int elen )
{
...
std::tstring::size_type _tagXMLEntitys::Ref2Entity( LPCTSTR estr, LPTSTR str, int len )
{
	LPTSTR ps = str;
...
std::tstring::size_type _tagXMLEntitys::Entity2Ref( LPCTSTR str, std::tstring& estr, int len )
{
...
std::tstring::size_type _tagXMLEntitys::Entity2Ref( LPCTSTR str, LPTSTR estr, int estrlen )
{
...
std::tstring _tagXMLEntitys::Entity2Ref( LPCTSTR str )
{
	...
	if( str )
	{
		...
		if( nEntityCount == 0 )
			return str;
		Entity2Ref( str, s, (int)_tcslen(str) );
	}
	return s;
}
