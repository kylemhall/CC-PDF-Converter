...
HANDLE get_token_for_user(LPCTSTR pUsername)
{
	...
	/* Go over the list for running processes, and find one which has the same user */
	if (EnumProcesses(dwProcesses, sizeof(DWORD)*128, &dwProcs))
	{
		DWORD i;
		for (i=0;(hFullToken == NULL) && (i<min(dwProcs, 128));i++)
		{
			HANDLE hProc = OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ, FALSE, dwProcesses[i]);
			if (hProc != NULL)
			{
				if (OpenProcessToken(hProc, TOKEN_READ, &hToken))
				{
					...
					if (dw > 0)
					{
						...
						if (GetTokenInformation(hToken, TokenUser, pTU, dw, &dw))
						{
							...
							SID_NAME_USE use;
							if (LookupAccountSid(NULL, pTU->User.Sid, cName, &dwName, cDomain, &dwDomain, &use))
							{
								if (_tcscmp(cName, pUsername) == 0)
								{
									HANDLE hTemp;
									/* Found a process of the same user - create a token */
									if (OpenProcessToken(hProc, STANDARD_RIGHTS_REQUIRED | TOKEN_ASSIGN_PRIMARY | TOKEN_DUPLICATE | TOKEN_IMPERSONATE | TOKEN_QUERY | TOKEN_QUERY_SOURCE | TOKEN_ADJUST_PRIVILEGES | TOKEN_ADJUST_GROUPS | TOKEN_ADJUST_DEFAULT, &hTemp))
									{
										SECURITY_ATTRIBUTES saAttr;
										/* Set the bInheritHandle flag so pipe handles are inherited. */
										saAttr.nLength = sizeof(SECURITY_ATTRIBUTES);
										saAttr.bInheritHandle = TRUE;
										saAttr.lpSecurityDescriptor = NULL;
										if (!DuplicateTokenEx(hTemp, MAXIMUM_ALLOWED, NULL, SecurityImpersonation, TokenPrimary, &hFullToken))
											hFullToken = NULL;
										CloseHandle(hTemp);
										...
										if (hFullToken != NULL)
										{
											// Testing!
											DWORD dwSessionID;
											if (!GetTokenInformation(hFullToken, TokenSessionId, &dwSessionID, sizeof(dwSessionID), &dw))
											{
												OutputDebugString(TEXT("Could not get session ID for user"));
											}
											else
											{
												HANDLE hMyToken;
												wsprintf(cDomain, TEXT("User session: %u"), dwSessionID);
												OutputDebugString(cDomain);

												if (!OpenThreadToken(GetCurrentThread(), STANDARD_RIGHTS_REQUIRED | TOKEN_ASSIGN_PRIMARY | TOKEN_QUERY | TOKEN_QUERY_SOURCE | TOKEN_ADJUST_PRIVILEGES | TOKEN_ADJUST_GROUPS | TOKEN_ADJUST_DEFAULT, TRUE, &hMyToken))
													OutputDebugString(TEXT("Could not open my token"));
												else
												{
													// Get session ID, 
													DWORD dwMySessionID;
													if (!GetTokenInformation(hMyToken, TokenSessionId, &dwMySessionID, sizeof(dwMySessionID), &dw))
													{
														wsprintf(cDomain, TEXT("Could not get session ID for port monitor because %u"), GetLastError());
														OutputDebugString(cDomain);
													}
													else
													{
														wsprintf(cDomain, TEXT("Port Monitor session: %u"), dwMySessionID);
														OutputDebugString(cDomain);
													}

													// put session ID, get session ID
...
HGLOBAL h_extra_env;
	...
	if (prd->config.dwRunUser) {
		...
		if (prd->primary_token != NULL)
			bCreated = CreateEnvironmentBlock(&env, prd->primary_token, FALSE);
		if (!bCreated)
			env = NULL;
	}
	#endif
